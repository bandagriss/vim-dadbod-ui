...
function! db_ui#dbout#update_param_table() abort
  let db_url = db#resolve(b:db)
  let parsed = db#url#parse(db_url)
  let scheme = db_ui#schemas#get(parsed.scheme)
  if empty(scheme)
...
function! s:recovery_primary_key(primary_key)
  let parsed = db#url#parse(db#resolve(b:db))
  let scheme = db_ui#schemas#get(parsed.scheme)
  if empty(scheme)
    return db_ui#notifications#error('Recovery headers not supported for '.parsed.scheme.' scheme.')
  endif
  ...
  let i = 0
  let lastcol = strlen(underline)
  let from_primary_key = 0
  let to_primary_key = 0
  while i <= lastcol
    if underline[i] !=? table_line || i == lastcol
      let to = i-1
      if (trim(column_line[from:to]) == a:primary_key)
        let from_primary_key = from
        let to_primary_key = to
        let i = lastcol
      endif
      let from = i+1
    endif
